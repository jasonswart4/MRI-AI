...
from functions_app import prepare_single_input_image, iou_loss, resize_image
import cv2
...
class SemanticSegmentationApp:
    def __init__(self, root):
        ...
        self.model = tf.keras.models.load_model("unet.h5", custom_objects={'iou_loss': iou_loss})
    ...
    def segment_image(self):
        ...
        net_input = prepare_single_input_image(image)
        net_input = np.array([net_input])

        # Perform segmentation using the model
        output = self.model.predict(net_input)

        muscle = resize_image(output[0][0], (a,b)) > 0.5
        fat = resize_image(output[1][0], (a,b)) > 0.5
        bone = resize_image(output[2][0], (a,b)) > 0.5

        muscle = muscle & ~bone
        fat = fat & ~muscle & ~bone
        bone = bone

        # Create an RGB mask based on the labels
        mask_r = 255*muscle
        mask_g = 255*fat
        mask_b = 255*bone

        mask_rgb = np.dstack((mask_r, mask_g, mask_b))
        print(np.shape(mask_rgb))
        return mask_rgb
